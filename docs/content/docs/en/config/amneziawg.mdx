...
| **S3**       | Cookie-reply padding, `0`-`64` — but at least `12` whenever a `HeaderProtectionKey` is set, which it is by default. |
| **S4**       | Transport (data) packet padding, `0`-`32` — same `12` floor as `S3` while a key is set. |
...
| **HeaderProtectionKey** | A base64 32-byte key for the 3.0 header-protection mechanism. Must match on every client config; blank disables it. Exactly 32 bytes, base64-encoded, on a single line — the panel refuses to save anything else, and a key pasted across two lines is rejected rather than silently truncated. Setting a key also raises the minimum for every one of `S1`-`S4` to `12`; since the panel generates a key by default, that floor is the normal case. |
...
| **RandomTrailers** | Appends a random number of bytes to the end of every packet. Must be enabled on both ends: the config carries it, so a 3.1-capable client turns it on itself, but an older client will not connect at all, and reports no error. On in a generated set. |
...
<Callout type="info">
  ...
  defaults, keep `H1`-`H4` **non-overlapping** and above `4` — nothing checks
  that for you, and an overlapping value keeps the embedded interface from
  coming up at all. Two other constraints are checked, and the panel refuses
  to save the inbound when either is broken: `S1 + 56 != S2`, and `S1`-`S4`
  all `>= 12` while a `HeaderProtectionKey` is set.
</Callout>
...
<Callout type="warn">
  ...
  clients still understand. `RandomTrailers` in particular has no wire
  negotiation: an older client hands back no error, it simply never completes
  a handshake. There is no host-side version requirement —
...
A client's downloadable `.conf` looks like this. The `vpn://` share link
carries the same settings, but not as this text: it is base64url of a
zlib-compressed Amnezia JSON container that the official apps import.
